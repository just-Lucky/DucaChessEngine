...
#include "eval.h"
    ...
    // Masck for updating castling rights based on the touched square
...
bool make_move(Board& board, Move move, UndoRecord& undo) {
    ...
    if (is_capture(move)) {
        if (flag == EP_CAPTURE) {
            ...
            captured = board.pieces[target];
        }
    }

    DirtyPiece dp;
    dp.dirtyNum = 1;
    dp.pc[0] = duca_to_nnue[piece];
    dp.from[0] = source;
    dp.to[0] = target;

    if (captured != -1) {
        dp.pc[dp.dirtyNum] = duca_to_nnue[captured];
        dp.from[dp.dirtyNum] = (flag == EP_CAPTURE) ? ((color == WHITE) ? target - 8 : target + 8) : target;
    ...
    if (flag >= PR_KNIGHT) {
        ...
        dp.pc[dp.dirtyNum] = duca_to_nnue[promoted_piece];
        dp.from[dp.dirtyNum] = 64;
        dp.to[dp.dirtyNum] = target;
        dp.dirtyNum++;
    } 
    else if (flag == KING_CASTLE || flag == QUEEN_CASTLE) {
        int rook_piece = (color == WHITE) ? W_ROOK : B_ROOK;
        int r_source = (flag == KING_CASTLE) ? ((color == WHITE) ? H1 : H8) : ((color == WHITE) ? A1 : A8);
        int r_target = (flag == KING_CASTLE) ? ((color == WHITE) ? F1 : F8) : ((color == WHITE) ? D1 : D8);
        
        dp.pc[dp.dirtyNum] = duca_to_nnue[rook_piece];
        dp.from[dp.dirtyNum] = r_source;
        dp.to[dp.dirtyNum] = r_target;
        dp.dirtyNum++;
    }
    
    board.dirty_piece = dp;
    ...
    board.pieces[target] = board.pieces[source]; 
    board.pieces[source] = -1;

    if (flag == EP_CAPTURE) {
        board.pieces[(color == WHITE) ? target - 8 : target + 8] = -1;
    } else if (flag >= PR_KNIGHT) {
        board.pieces[target] = final_piece_on_target;
    } else if (flag == KING_CASTLE) {
        if (color == WHITE) { board.pieces[H1] = -1; board.pieces[F1] = W_ROOK; }
        else                { board.pieces[H8] = -1; board.pieces[F8] = B_ROOK; }
    } else if (flag == QUEEN_CASTLE) {
        if (color == WHITE) { board.pieces[A1] = -1; board.pieces[D1] = W_ROOK; }
        else                { board.pieces[A8] = -1; board.pieces[D8] = B_ROOK; }
    }
