...
static Move counter_move_table[64];
...
int score_move(const Board& board, Move move, Move tt_move, int ply, Move prev_move) {
    ...
    // Priority 3: Quiet Moves
        ...
        if (ply < 64 && killer_moves[1][ply] == move) {
            return 7000;
        }
        if (prev_move && counter_move_table[get_target(prev_move)] == move) {
            return 6500;
        }
        else {
...
int quiescence_search(Board& board, int alpha, int beta, int ply) {
    ...
    for (int i = 0; i < move_list.count; ++i) {
        move_scores[i] = score_move(board, move_list.moves[i], 0, ply, 0);
    }
...
int alpha_beta(Board& board, int alpha, int beta, int depth, int ply, Move prev_move) {
    if ((nodes_searched + quiescence_nodes) % 2048 == 0) {
        if (search_time_limit != -1) {
            auto now = std::chrono::high_resolution_clock::now();
            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - search_start_time).count();
    ...
    if (depth >= 3 && ply > 0 && !in_check && static_eval >= beta) {
        ...
        if(non_pawns != 0){
            ...
            int R = 3 + (depth / 6);
            nnue_thread_state[ply + 1].accumulator.computedAccumulation = 0;
            int null_score = -alpha_beta(board, -beta, -beta + 1, depth - 1 - R, ply + 1, 0);
    ...
    int move_scores[256];
    for (int i = 0; i < move_list.count; ++i) {
        move_scores[i] = score_move(board, move_list.moves[i], tt_move, ply, prev_move);
    }
    ...
    for (int i = 0; i < move_list.count; ++i) {
        ...
        if (legal_moves == 1) {
            nnue_thread_state[ply + 1].accumulator.computedAccumulation = 0;
            score = -alpha_beta(board, -beta, -alpha, depth - 1 + extension, ply + 1, move);
        } else {
            int reduction = 0;
            // Late Move Reduction
            ...
            nnue_thread_state[ply + 1].accumulator.computedAccumulation = 0;
            score = -alpha_beta(board, -alpha - 1, -alpha, depth - 1 - reduction + extension, ply + 1, move);
            
            if (score > alpha && reduction > 0) {
                nnue_thread_state[ply + 1].accumulator.computedAccumulation = 0;
                score = -alpha_beta(board, -alpha - 1, -alpha, depth - 1 + extension, ply + 1, move);
            }
...
void search_position(Board& board, int max_depth) {
    ...
    memset(counter_move_table, 0, sizeof(counter_move_table));
    root_best_move = 0;
    ...
    for (int current_depth = 1; current_depth <= max_depth; ++current_depth) {
        ...
        if (current_depth >= 5) {
            ...
            while (!search_aborted) {
                score = alpha_beta(board, alpha, beta, search_depth, 0, 0);
            ...
            score = alpha_beta(board, -INF_VAL, INF_VAL, current_depth, 0, 0);
        }
        last_score = score;
