...
// Helper that converts internal 16-bit move into UCI format
...
// Helper that parses UCI string and finds corresponding move
...
void parse_go(Board& board, std::stringstream& ss, int num_search_threads) {
    std::string token;
    int depth = 64; // Engine's max depth
    ...
    search_thread = std::thread(search_position, std::ref(board), depth, num_search_threads);
}

void uci_loop(int num_search_threads) {
    setbuf(stdout, NULL);
    setbuf(stdin, NULL);
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (command == "uci") {
            ...
            parse_go(board, ss, num_search_threads);
        } 
